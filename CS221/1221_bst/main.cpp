...
int main()
{
    ...
    for (int i=0;i<opcount;++i)
    {
        ...
        switch(op[0])
        {
            case 'i': t.insert(param);break;
            case 'd':
                if (op[6]){
                    switch (op[7])
                    {
                        case 'i': cin >> param2;
                                  t.delete_interval(param,param2);
                                  break;
                        case 'l': t.delete_less(param);break;
                        case 'g': t.delete_more(param);break;
                    }
                    break;}
                else { t.remove(param); break; }
            case 'f':
                if (op[4]!='\0')
                {
                    int res;
                    res = t.find_ith(param);
                    if (res) cout << res << endl;
                    else cout << 'N' << endl;
                    break;
                }
                else {cout << ((t.find(param)==1)?'Y':'N') << endl;break;}
        }
 //       t.midOrder();
  //    cout << endl;
    }
